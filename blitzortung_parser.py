...
def decompress_ws(data: bytes) -> str:
    """
    Decompress Blitzortung permessage-deflate WebSocket frames.
    Raw DEFLATE = wbits = -15.

    NOTE:
    - Some Blitzortung servers may NOT actually use permessage-deflate.
    - In that case, the text you get back is already the "weird" LZW-style
      encoded string, not zlib-compressed bytes.
    """
    try:
...
def blitzortung_lzw_decode(encoded: str) -> str:
    """
    Decode Blitzortung's obfuscated WebSocket payload into a plain JSON string.

    This is a dictionary-based LZW-style string encoding:

    - Codes 0–255 are literal characters.
    - Codes >= 256 refer to dictionary entries that are built on the fly.
    - When a code >= 256 is not yet in the dictionary, the decoder uses the
      classic LZW "KwKwK" rule: previous_string + first_char(previous_string).

    **Important:** This is *not* cryptographic encryption. It's a lightweight
    string compression/obfuscation scheme layered on top of JSON.
    """
    if not encoded:
        return ""
...
def decode_binary_value(value):
    """
    Attempt to decode binary-encoded values that appear as high-byte UTF-8 characters.
    This is a heuristic approach - may still be useful even after LZW decode, if
    some fields store packed integers.
    """
...
def clean_lightning_data(data):
    """
    Recursively clean and decode the lightning strike data structure.
    """
...
def parse_lightning_message(message_str: str) -> dict:
    """
    Parse and decode a Blitzortung lightning message.

    Behavior:
    1) Try to parse `message_str` directly as JSON (plain text frame).
    2) If that fails with JSONDecodeError, assume it's Blitzortung's
       custom LZW-style string encoding and try blitzortung_lzw_decode(),
       THEN parse the result as JSON.

    Returns a dictionary:
        {
            "success": True/False,
            "raw": <raw JSON dict>        (on success)
            "decoded": <cleaned dict>,    (on success)
            "encoding": "plain_json" or "blitzortung_lzw_string_obfuscation",
            "error": "...",               (on failure)
            "raw_message": "..."          (truncated raw text on failure)
        }
    """
